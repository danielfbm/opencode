import { useParams, Navigate, useNavigate } from "@solidjs/router"
...
export default function InvestigationDetails() {
  ...
  const openFullSession = () => {
    const dir = directory()
    const sessId = investigation()?.sessionId
    const invId = params.id
    if (dir && sessId) {
      navigate(`/${base64Encode(dir)}/session/${sessId}?inv=${invId}`)
    }
  }
            ...
            <div class="flex items-center gap-3">
              <Show when={investigation()?.sessionId}>
                <button
                  ...
                  class="flex items-center gap-1.5 text-xs text-[var(--aui-color-primary)] hover:text-[var(--aui-color-primary-hover)] transition-colors"
                >
